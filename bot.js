...
const ONB = {
  main: {
    text:
      `📚 *Онбординг саппорта + докопса*\n\n` +
      `привет 💙\n` +
      `тут коротко про то, кто за что, где что лежит и как у нас всё устроено.\n` +
      `посмотри один раз — и будешь ориентироваться.`,
    kb: () => new InlineKeyboard()
      .text("📋 Часть 1: Вход", "onb:part1").row()
      .text("🗣 Часть 2: Как говорим", "onb:part2").row()
      .text("⚙️ Часть 3: Ежедневная работа", "onb:part3"),
  },
  ...
  part1: {
    text:
      `📋 *Часть 1 — Вход*\n\n` +
      `первые шаги — доступы, инструменты и кто есть кто.`,
    kb: () => new InlineKeyboard()
      .text("✅ Чеклист доступов", "onb:p1:access").row()
      .text("🛠 AI Setup", "onb:p1:tools").row()
      .text("👥 Команда: кто за что", "onb:p1:team").row()
  ...
  "p1:access": {
    text:
      `✅ *чеклист доступов*\n\n` +
      `тебе понадобятся:\n` +
      `• telegram-чат саппорта — попроси у Юли (@matsako)\n` +
      `• github-репо (matsako/gconf) — инвайт от @matsako\n` +
      `• linear — команды Gconf\\_support и gconf\\_docops\n` +
      `• админка — инвойсы и управление пользователями\n` +
      `• MASTER\\_INTAKE (google sheet) — таблица докопса\n` +
      `• quick replies в telegram — шаблоны частых ответов\n\n` +
      `🔑 *где пароли и доступы:*\n` +
      `gconf files (telegram, support-чат)\n\n` +
      `📌 там может быть немного хаос из старого и нового — это нормально, разберёшься по ходу`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 1", "onb:part1"),
  },
  "p1:tools": {
    text:
      `🛠 *AI Setup*\n\n` +
      `минимальный набор, с которым мы работаем:\n\n` +
      `📝 *Cursor* — $20/мес\n` +
      `AI-редактор. даже если ты не программист — пригодится для текстов, промптов, файлов.\n` +
      `корп. аккаунт: попроси Лёшу (@atassist)\n\n` +
      `⌨️ *Claude Code* — $20/мес\n` +
      `работает через терминал внутри Cursor. через терминал гораздо больше возможностей.\n\n` +
      `📦 *GitHub-репо*\n` +
      `приватный репозиторий. артефакты, промпты, кейсы — можно в любой момент подтянуть к себе в Cursor.\n\n` +
      `🤖 *этот бот*\n` +
      `/todo — задачи, /help — справка. просто напиши текст или надиктуй голосовое — задача улетит в Linear.`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 1", "onb:part1"),
  },
  "p1:team": {
    text:
      `👥 *команда: кто за что*\n\n` +
      `*Лёша Травкин* — ассистент Хакунова\n` +
      `→ возвраты, платёжные системы, подпись на инвойсе, акты\n\n` +
      `*Лера* — ЭДО\n` +
      `→ электронный документооборот\n\n` +
      `*Полина* — договоры\n` +
      `→ серьёзные вопросы по документообороту и юрлицам\n\n` +
      `*Наташа Дзера* — опс\n` +
      `→ операционные вопросы gconf\n\n` +
      `*Юля Мацако* — CEO\n` +
      `→ стратегия, доступы к репо и инструментам\n\n` +
      `📌 девов уже нет. circle больше нет — у нас своя LMS. саппорт = один человек на оба потока.`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 1", "onb:part1"),
  },
  ...
  part2: {
    text:
      `🗣 *Часть 2 — Как говорим*\n\n` +
      `тон — это главное. тут про то, как мы звучим и почему это важно.`,
    kb: () => new InlineKeyboard()
      .text("🎯 Тон и стиль", "onb:p2:tone").row()
      .text("✏️ Чеклист перед отправкой", "onb:p2:check").row()
      .text("📖 Где учиться тону", "onb:p2:materials").row()
      .text("⬅️ Назад", "onb:back"),
  },
  "p2:tone": {
    text:
      ...
      `💡 можешь проверить свой тон через Cursor:\n` +
      `«проверь сквозь 10-support что мой ответ написан в нашем тоне»`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 2", "onb:part2"),
  },
  "p2:check": {
    text:
      `✏️ *чеклист перед отправкой*\n\n` +
      `быстрая самопроверка — можно прогнать перед тем как отправить:\n\n` +
      `• спокойная уверенность без позы?\n` +
      `• короткие фразы, одна мысль на строку?\n` +
      `• обращение на «ты»?\n` +
      `• без лишних обещаний и дат?\n` +
      `• emoji только по делу?\n` +
      `  📍 рамка · 📌 пояснение · 💙 тепло · 🔥 энергия · ☝️ внимание\n\n` +
      `если да — стиль сохранён 💙`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 2", "onb:part2"),
  },
  "p2:materials": {
    text:
      `📖 *где учиться тону*\n\n` +
      `всё лежит в репо, открой в Cursor:\n\n` +
      `• *плейбук:* 10-support/Playbook/support\\_playbook\\_EN.md\n` +
      `• *grand verbatim:* 10-support/data/grand-verbatim-base.md\n` +
      `  (100+ реальных диалогов с клиентами — это наш source of truth)\n` +
      `• *FAQ:* 10-support/FAQ/faq\\_inner/FAQ\\_V1.md\n\n` +
      `📌 grand verbatim обновлён до января 2026.\n` +
      `telethon перестал пускать выгрузку.\n` +
      `структура готова — загрузи новые чаты и запусти /update-support-kb в Cursor.`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 2", "onb:part2"),
  },
  ...
  part3: {
    text:
      `⚙️ *Часть 3 — Ежедневная работа*\n\n` +
      `как устроен обычный день — кейсы, докопс, обновление базы знаний.`,
    kb: () => new InlineKeyboard()
      .text("🔄 Как обрабатываю кейс", "onb:p3:case").row()
      .text("📄 DocOps flow", "onb:p3:docops").row()
      .text("📋 SOP: быстрый справочник", "onb:p3:sop").row()
      .text("🧠 Как обновлять KB", "onb:p3:kb").row()
      .text("⬅️ Назад", "onb:back"),
  },
  "p3:case": {
    text:
      `🔄 *как обрабатываю кейс*\n\n` +
      `1. ищу ответ в FAQ\\_V1.md\n` +
      `2. нет? → tech\\_support\\_guides.md\n` +
      `3. всё ещё нет? → ищу похожий кейс в grand-verbatim через Cursor\n` +
      `4. адаптирую формулировку под человека, отправляю\n` +
      `5. новый паттерн? → добавляю в FAQ\n\n` +
      `*если не знаешь — не выдумывай, спроси:*\n` +
      `• возвраты, инвойсы → Лёша Травкин\n` +
      `• документы, юрлица → Полина\n` +
      `• ЭДО → Лера\n` +
      `• стратегия → Юля`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 3", "onb:part3"),
  },
  "p3:docops": {
    text:
      `📄 *DocOps: документооборот*\n\n` +
      `как это сейчас работает (март 2026):\n\n` +
      `клиент → запрос на оплату/инвойс/доступ\n` +
      `  ↓\n` +
      `support → заносит в MASTER\\_INTAKE\n` +
      `  ↓\n` +
      `инвойс → вручную в админке\n` +
      `  ↓\n` +
      `статусы → таблица + Linear\n` +
      `  ↓\n` +
      `Травкин/Оля → смотрят 2 раза в неделю\n\n` +
      `📌 докопс растёт ~20% каждый месяц\n` +
      `🔧 в работе: автозаполнение инвойсов, перевод в Linear`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 3", "onb:part3"),
  },
  "p3:sop": {
    text:
      `📋 *SOP: быстрый справочник*\n\n` +
      `полная версия: 10-support/SOP/SOP.md\n\n` +
      `*PAY-01 · оплата через бота*\n` +
      `автоматический сценарий. бот сам выдаёт доступ — ничего делать не нужно.\n\n` +
      `*PAY-02 · оплата не через бот*\n` +
      `промокоды, ручные случаи → проверяешь поступление → выдаёшь доступ.\n\n` +
      `*ACC · доступы*\n` +
      `проверяешь оплату → добавляешь в систему → подтверждаешь клиенту.\n\n` +
      `формат каждого SOP:\n` +
      `TL;DR → WHEN → WHO → FLOW → ALT → DO NOT → DONE`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 3", "onb:part3"),
  },
  "p3:kb": {
    text:
      `🧠 *как обновлять knowledge base*\n\n` +
      `в Cursor / Claude Code запусти:\n` +
      `/update-support-kb\n\n` +
      `что он сделает:\n` +
      `1. посмотрит структуру 10-support/\n` +
      `2. возьмёт новые чаты из data/raw/\n` +
      `3. обновит FAQ, grand verbatim\n` +
      `4. даст отчёт\n\n` +
      `*куда класть новые данные:*\n` +
      `сырые чаты → 10-support/data/raw/\n` +
      `это чаты общения с клиентами из telegram (markdown или JSON)\n\n` +
      `*что обновлять руками:*\n` +
      `• FAQ — когда появился новый повторяющийся вопрос\n` +
      `• SOP — когда изменился процесс\n` +
      `• playbook — когда тон эволюционирует (осторожно 💙)`,
    kb: () => new InlineKeyboard().text("⬅️ Часть 3", "onb:part3"),
  },
};
